...
const AMDCBPCOffer = () => {
  ...
  const handleSubmit = async () => {
    const { error } = await supabase
      ...
      .match({ build_type: "AMD" }); // Only update where build_type is 'AMD'

    if (error) {
      toast.error("Error applying offer.");
    } else {
      toast.success("Offer applied on AMD build PCs successfully.");
  ...
  return (
    <div className="w-full pt-6 flex flex-col items-center p-6 gap-6">
      ...
      <h1 className="text-2xl font-bold">
        Offer for{" "}
        <span className="text-indigo-500">Custom AMD-Build Products</span>
      </h1>
      <p className="text-medium p-6 text-center">
        ...
        <span className="text-indigo-500">Custom AMD-Build Products.</span> You
        will also need to enter the{" "}
      ...
      <div className="w-96 rounded-md flex flex-col justify-center items-center p-4 bg-gray-900 gap-6">
        ...
        <div className="w-full flex gap-2 justify-between items-center">
          ...
          <div className="w-[60%] rounded-md bg-slate-700 flex justify-center items-center gap-4 p-2">
            <Image
